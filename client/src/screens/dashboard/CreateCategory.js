    ...
    return (
        <Wrapper>
            ...
            <form className="w-full md:w-8/12" onSubmit={submitCategory}>
                <h3 className="text-xl capitalize mb-3"> create category</h3>
                {errors.length > 0 && errors.map((error, key) => (
                    <p className="alert-danger" key={key}>{error.msg} </p>
                ))}
                <div className="mb-3">
                    <input type={'text'} name="" className="form-control" placeholder="Enter category name" value={state} onChange={(e) => setState(e.target.value)} />
                </div>
                <div className="mb-3">
